...
2. Apply `workflow/chatgpt_only/RESEARCH.md#Final Return-target protocol` before changing anything. If `Return reconciliation: applied`, verify its exact result refs, perform only consume/clear, return to the router, and do **not** enter correction logic.
3. Recover current durable implementation/result/review state. If the required correction/reconciliation is already durably persisted but reconciliation is still `pending`, do **not** repeat it; finalize the applied marker/result refs around that proven result. A matching downstream `pending | in_progress | green | red` review state for the corrected exact subject is downstream evidence that the old Research-return correction has already crossed its review boundary; it does not reactivate the old correction.
...
6. `execution_resolution` classifies the findings and either durably refines the record's Return target to the exact final owning role/subject while keeping `Status: complete` and the Task Board pointer, or, if classification itself needs more evidence, performs the atomic classifier-to-Research chain transition from `RESEARCH.md` so the pointer moves directly from consumed `R1` to active `R2`;
7. when a final target is selected, that final target performs its correction/reconciliation; only after the result is durably persisted does it set Research to `consumed` and clear Task Board `research_obligation`.
