...
- create one exact obligation under `workflow/chatgpt_only/RESEARCH.md#Durable record contract` with `Origin role: project_definition`, the exact current Definition subject/revision as Origin subject, `Return target: project_definition:<exact subject>`, and `Return reconciliation: pending`;
...
When returning from a `complete` Research record for this Definition subject, follow `workflow/chatgpt_only/RESEARCH.md#Final Return-target protocol`:
- reconcile verified findings into requirements/decisions/open questions as appropriate;
- in the same durable Git transition as those target mutations, set `Return reconciliation: applied` and exact result refs;
- only after that transition is durable may the record be consumed and its owning pointer cleared;
- recovery from `applied + complete` is consume/clear-only and must not repeat Definition mutation.
