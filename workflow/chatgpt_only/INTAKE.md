...
2. Discover only relevant active branches, PRs and workstream manifests needed for identity/dependency/base classification. No mutable global workstream registry is required.
3. Detect whether the request already targets an existing exact workstream through a durable locator such as branch, workstream ID, manifest or PR. If yes, recover that workstream instead of creating a duplicate.
4. Choose the exact integration target and base under **Base and dependency classification** below.
...
8. Perform the route-specific intake below.
...
1. Establish the problem on the intended baseline when practical before implementation mutation. Prefer reproduction, a failing check, exact source/runtime evidence, or another concrete diagnostic signal.
...
3. Select independent versus stacked base using the common rules above.
4. Create/recover the issue workstream and durable active intake state before code/config/runtime implementation mutation.
5. Determine whether the issue qualifies for the micro-fix path using the accepted R6 criteria:
...
6. If all micro-fix criteria hold, record `path: micro_fix`. M03 owns the actual durable micro-fix execution/review contract; Intake must not weaken or substitute for it.
7. Otherwise record the smallest normal route justified by evidence:
...
8. Materialize the selected route's canonical durable starting state before marking intake complete.
9. Return to the router.
...
2. Select the normal integration target unless a real stacked dependency exists.
3. Create/recover the feature workstream and durable active intake state.
4. Preserve the existing exploratory lifecycle. Before marking intake complete, create/reconcile the exact canonical Brainstorming starting record/pointer required by the ChatGPT-only Brainstorming contract on this workstream branch.
5. Initialize/retain Definition promotion authorization as `pending` with promotion subject `none` unless the user separately and explicitly authorizes promotion for the exact current brainstorming scope/revision under the normal router gate.
6. Do **not** interpret the `#feature` directive itself as Definition promotion authority.
7. Mark intake complete only after durable discovery state is recoverable, then return to the router. Normal Brainstorming/Research → explicit promotion → Definition → Planning semantics continue from there.
