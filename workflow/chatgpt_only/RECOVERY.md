...
- `workflow/chatgpt_only/INTAKE.md` only when historical root/default migration must recover or create workstream identity/topology;
...
2. Recover one exact migration topology **before branch creation or adoption**. First recover any existing branch/manifest/PR that durably represents this same obligation. Otherwise recover the repository's intended normal `integration_target` and the exact creation base from durable project/Git/PR evidence, then apply `INTAKE.md#Base and dependency classification`: use an independent workstream with null parent fields when no parent-only dependency exists; use a stacked workstream only when exact evidence proves a required parent workstream/branch and record its `parent_dependency`. The selected base must be the exact integration-target base for independent work or the exact required parent-only base for stacked work. If `integration_target`, exact base, parent ownership or dependency classification cannot be proven, fail closed before creating/adopting a branch.
3. Establish one exact migration identity. Reuse the exact coherently recovered workstream identity when one already exists. Otherwise use neutral `kind: change` identity and the deterministic `change-<slug>` / `work/<slug>` collision rules from `INTAKE.md#Workstream identity and naming`. A pre-existing non-target branch may be adopted only when exact Git + durable state prove it owns this same obligation **and** establish coherent `base_ref`, `integration_target` and parent metadata; otherwise do not guess or claim it.
4. Ensure the exact workstream branch exists **before** writing migrated managed state. When creating a new branch, create it from the exact base selected in step 2. Materialize/reconcile one `WORKSTREAM.yaml` plus one namespaced `TASK_BOARD.yaml` on that branch, persisting `base_ref`, `integration_target`, `parent_workstream`, `parent_branch` and `parent_dependency` consistently with the proven topology, plus exact manifest `id/branch/task_board` ↔ Task Board `workstream_id/execution_ref.branch` binding. Do not create a second lane when a partial migration already materialized the same identity/topology.
5. Migrate only continuation truth required for coherent recovery: current plan/milestone identity, live/non-terminal Cards and required dependency results, exact Card/milestone review fields, implementation/recovery Research pointer, result/evidence/test pointers, blockers and execution provenance. Preserve immutable historical contracts/evidence/handoffs by exact reference when they remain valid; do not rewrite or duplicate completed history merely for layout.
6. If historical root `PROJECT.md` also carries an unreconciled pre-execution exploratory/Research locator for this same obligation, migrate that exact locator into the selected manifest `routing.*` ownership defined by M02-T01. Do not mirror lifecycle fields and do not leave two active pointers.
7. Persist concise migration provenance when the mapping is non-trivial, including the exact source root/default state ref, proven target/base/dependency classification and the resulting workstream manifest/Task Board ref. The source root/default board remains historical input and is not cleared, advanced or used as a mutable owner merely to mark migration complete.
8. Read back the workstream branch and require successful manifest ↔ Task Board binding, exact manifest topology matching the proven `integration_target` / `base_ref` / parent classification, and exact preservation of every still-live review/Research/result dependency. Only after that readback is GREEN may the namespaced Task Board become the selected mutable execution state and normal recovery priority above apply.
9. For a long-lived legacy branch whose root-state files would overwrite independently evolved target-side historical/default files at final integration, apply `REPOSITORY.md#Legacy branch → branch-isolated finalization migration` after active ownership is namespaced. Any later root-file reconciliation there is conflict-avoidance/history preservation, never resumption of root/default execution.
10. If source identity, branch ownership, current live obligation, integration target/base/dependency classification or migration mapping remains ambiguous, preserve both source and any partial target state and fail closed. Ask for user input only when exact durable/Git evidence cannot resolve the ambiguity.

Crash rule: before step 8 succeeds, retry/recover the same deterministic migration identity **and topology** without mutating the source root board. After step 8 succeeds, never route the historical root board as active state; recover the namespaced workstream instead.
