...
1. create a small `PROJECT.md` with the accepted `execution_policy: codex_only`;
...
5. use Project Definition to promote accepted intent into `requirements/` + `decisions/` only after the codex_only promotion gate is satisfied;
6. create an approved Master Plan only after Definition Complete is GREEN;
7. create Task Board from `workflow/codex_only/TASK_BOARD_TEMPLATE.yaml` and create Task Cards just-in-time through Execution Prep;
