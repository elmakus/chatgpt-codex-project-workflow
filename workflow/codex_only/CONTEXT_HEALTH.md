# Codex-only Context Health Gate

This module protects execution quality when the coordinating Codex Main project context has accumulated enough stale or irrelevant project context that a clean repository-backed restart is safer than continuing. Concrete worker/session lifecycle remains owned by `codex_workflow`.
...
If any real human/project stop already owns the boundary — unresolved strategic/product authority, explicit authorization, concrete unremediable runtime/input blocker, or end of approved scope — use that boundary instead. Formal Tester review is runtime-realized and is not itself a mandatory normal-ChatGPT hygiene stop.
...
Continue in the same coordinating context when current context is still a net help and durable authority can be followed without material ambiguity.
...
- the coordinating context contains materially superseded/contradictory state that could plausibly influence the next obligation;
...
- the coordinating context has crossed several materially different roles or major project phases and much of the prior context is no longer relevant;
...
4. verify that a fresh coordinating context can recover everything needed from durable state without this transcript;
...
The new coordinating context reconstructs project state from current workflow `main` + durable repository authority and resumes the exact next obligation. Worker resume/replacement remains a separate runtime concern.
