...
6a. For a selected branch-first workstream, validate the manifest orchestration schema before any route may realize a policy-dependent runtime role. An active Intake immediately after initial manifest materialization may temporarily own the template's null binding while it establishes it under `INTAKE.md`. Outside that exact initial transition, a missing `orchestration` block is the bounded pre-schema Recovery case, while a present binding with missing/unknown/contradictory required values is invalid and routes fail-closed to Recovery rather than being treated as legacy absence.
6b. After known coordinator context loss/compaction/repository-only reconstruction, or whenever Main cannot positively establish current-context re-bind, treat the non-durable binding latch as absent/uncertain. Load `ORCHESTRATION_KERNEL.md` conditionally at that boundary and require the shared pre-dispatch gate in `workflow/codex/CODEX_ORCHESTRATION.md` before the first policy-dependent realization/re-realization. Do not reload this recovery material on ordinary role transitions while the latch remains positively established.
