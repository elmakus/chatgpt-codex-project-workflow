...
This contract owns explicit operator intake for new branch-isolated Codex-only workstreams.
...
The marker selects the **entry route only**. After intake has durably established the workstream and downstream state, return to `workflow/codex_only/ROUTER.md`. The marker is not a permanent session-scope boundary.
