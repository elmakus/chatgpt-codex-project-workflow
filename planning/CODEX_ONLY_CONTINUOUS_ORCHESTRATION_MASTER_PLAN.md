...
3. Remove/reword `workflow/codex_only/PLANNING.md` language that permits `coordinating-context refresh boundaries` so strategic plans cannot schedule coordinator hygiene as a Project Workflow stop under `codex_only`.
...
5. Add a dedicated regression contract test for `codex_only` continuous orchestration covering Router plus the active Planning/Close/Recovery/orchestration contracts, and preserve the existing ChatGPT-only Context Health test unchanged.
