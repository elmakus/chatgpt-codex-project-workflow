...
- OpenSpec candidate: **yes, JIT and minimal** — this changes a shared behavior contract across policy routes. Execution Prep should create/reconcile one small OpenSpec change only if the current OpenSpec policy still judges freezing the behavior contract useful; no separate design document unless current implementation risk actually warrants it.
...
1. JIT-confirm whether one compact OpenSpec change is warranted by the current shared behavior-contract risk.
...
- Execution Prep decides from actual current source whether freezing a compact proposal/spec/tasks adds useful precision.
...
- OpenSpec boundary: GREEN — candidate only, JIT-selective rather than mandatory ceremony.
...
Freeze this exact `YAGNI-P1` draft for independent plan review before approval. Any substantive correction after review creates a new plan revision/subject.
