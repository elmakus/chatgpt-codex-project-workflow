...
Plan revision: P5
Planning cycle: 5 (entry subject `definition:R3|planning-cycle:5`)
Lifecycle: owned by `PLANNING.toml`; this P5 text is the immutable review
...
Predecessor: cycle 4 / P4 exact frozen GREEN-reviewed approved subject
...
Replan authority/derivation: Definition R3 (`pwv21-policy-kernel@3`) and its
accepted R3 requirements/ADR deltas. P5 preserves P4 strategy and terminal
M01/M02 history while incorporating semantic Card right-sizing, Worker
falsification/YAGNI discipline, corrected discovery-vs-closure convergence,
per-defect-class repair ceilings, and durable non-load-bearing review
observation reconciliation before M02R materialization. No M01/M02 reopen or
M03 materialization is introduced by this replan.
...
- Planning record:
  ...
  (cycle 5 / P5; that record alone owns mutable lifecycle). Earlier P1–P4
...
P5 uses one bounded bootstrap correction milestone, `M02R`, between the
...
Therefore:
- M02R is implemented under current canonical V2 governance, with P5 itself
...
Authority input pins for P5 (consumer branch state containing Definition R3):
...
**P4 lifecycle note:** historical terminal milestone. P4 does not reopen or
...
implemented by this milestone, P5 freezes four **required_seam** boundaries
...
1. BOOT-A: extend review contracts/state/tests so formal lifecycle distinguishes
   finding-closure verification from fresh full-scope discovery; enforce
   complete-pass material finding collection, class-level/root-cause repair,
...
classification against the governed P5 materialization. No special user prompt
...
- Historical GREEN results and correctly closed stages are never
  ...
  (REQ-098/099/125).
- M01/M02 remain terminal. P5 does not reopen them. M02 is additionally
  replayed as immutable regression evidence by M02R.
- The existing waiting `after-M02-T01` JIT state must be reconciled after
  P5 approval: it must not materialize M03 directly. Execution Prep first
...
Full P5 audit evidence:
`implementation/workstreams/change-pwv21-policy-kernel-brainstorming/evidence/PLANNING_AUDIT_P5_2026-09-24.md`.

- Definition R3 is GREEN with 137 accepted requirements and 8 accepted ADRs.
- Premium A for `definition:R3|planning-cycle:5` was satisfied before
  material P5 planning.
- M01/M02 remain terminal and are not rematerialized.
- P5 preserves P4 milestone topology; because M02R was not materialized, R3
...
- The waiting `after-M02-T01` trigger cannot materialize M03 directly; after
  P5 approval it must be reconciled to M02R-first execution with four required
  bootstrap seams.
- P5 preserves authority-level dogfood vs deployed workflow enforcement while
