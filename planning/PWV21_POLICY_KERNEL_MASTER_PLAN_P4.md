...
Plan revision: P4
Planning cycle: 4 (entry subject `definition:R2|planning-cycle:4`)
Lifecycle: owned by `PLANNING.toml`; this P4 text is the immutable review
...
Predecessor: cycle 3 / P3 exact frozen RED subject
...
P4 preserves P3 strategy and all 127 requirement ownerships while correcting
exactly the two material P3/R01 findings: the accepted 5/4/3 review-ceiling
authority boundary is no longer delegated to JIT, and ADR-PWV21-006/007 are
explicitly included in ADR coverage. No Definition change, Research, M02
reopen, or M03 materialization is introduced by this correction.
...
- Planning record:
  ...
  (cycle 4 / P4; that record alone owns mutable lifecycle). Earlier P1/P2
...
P4 uses one bounded bootstrap correction milestone, `M02R`, between the
...
Authority input pins for P4 (consumer branch state containing Definition R2):
...
by this milestone, P4 freezes three **required_seam** boundaries for M02R.
...
classification against the governed P4 materialization. No special user prompt
...
within accepted semantics. The default hard review ceilings are accepted
authority and are NOT a JIT choice: Card=5, Milestone=4, final-integration=3
per stable epoch, with bounded finding-verification passes excluded. A plan
may lower a ceiling only through explicit accepted Strategic Planning
rationale; raising an accepted default requires a material authority/planning
revision rather than Execution Prep refinement. Any weakening of freshness,
evidence gating, the accepted ceiling/mode-switch semantics, or routing
discipline returns to Strategic Planning/Definition as appropriate.
...
| ADR-PWV21-006 | Preserve planner decomposition intent through JIT; risk-based topology challenge | M02R (generic seam/topology semantics), M03 (first downstream seam dogfood) |
...
- Review-ceiling authority (REQ-112/113/114): M02R establishes the stable
  epoch accounting and accepted default hard ceilings Card=5, Milestone=4,
  final-integration=3; M05 integrates those fixed defaults into the complete
  review lifecycle but cannot redefine them through JIT. Ceiling hit changes
  mode to convergence analysis and never converts RED to GREEN.
...
Full P4 audit evidence:
`implementation/workstreams/change-pwv21-policy-kernel-brainstorming/evidence/PLANNING_AUDIT_P4_2026-09-24.md`.
...
- P3/R01 F1 is closed: M05 no longer delegates numeric review ceilings to
  JIT; accepted defaults remain Card=5/Milestone=4/final=3.
- P3/R01 F2 is closed: ADR coverage explicitly maps ADR-PWV21-006 and
  ADR-PWV21-007 to M02R/M03.
- M03 cannot materialize before M02R terminal GREEN.
- P4 explicitly distinguishes authority-level dogfood from deployed
