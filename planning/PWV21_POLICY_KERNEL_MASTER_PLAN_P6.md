...
Plan revision: P6
Planning cycle: 6 (entry subject `definition:R3|planning-cycle:6`)
Lifecycle: owned by `PLANNING.toml`; this P6 text is the immutable review
...
Predecessor: cycle 5 / P5 exact frozen RED-reviewed subject
...
Replan authority/derivation: Definition R3 (`pwv21-policy-kernel@3`) plus
independent P5/R01 RED evidence
`implementation/workstreams/change-pwv21-policy-kernel-brainstorming/evidence/PLAN_REVIEW_P5_R01_2026-09-24.md`.
P6 preserves P5 strategy, milestone topology, requirement ownership and
terminal M01/M02 history while correcting the material REQ-108
finding-closure causal-blast-radius coverage defect. It also resolves the two
non-blocking P5 review observations by removing stale current-plan wording and
making REQ-061 closure-verification wording exact. No M01/M02 reopen or M03
materialization is introduced by this replan.

## Authority

- Requirements: `requirements/PWV21_POLICY_KERNEL.md` R3 (status `approved`,
  Definition subject `pwv21-policy-kernel@3`, 137 accepted requirements
  PWV21-REQ-001…PWV21-REQ-137)
...
- Fixed small/medium/large project classes, LOC/file/token/time ceilings, or
  a PWv2.1 wall-clock optimization policy.

## Strategy overview

P6 uses one bounded bootstrap correction milestone, `M02R`, between the
...
R2+R3 workflow-semantic corrections that must exist before downstream dogfood.
...
| M02R R2+R3 workflow-semantic bootstrap | M02 | Definition R3 + terminal M02 evidence | Implements review convergence/observation closure, semantic Card right-sizing/decomposition fidelity, Worker discipline and live-validation handling before downstream JIT |
...
Therefore:
- M02R is implemented under current canonical V2 governance, with P6 itself
  freezing the bootstrap Card seams needed to prevent another mega-Card or meaningless micro-Card;
- M03 is the first **authority-level dogfood** of the complete R2+R3 decomposition/right-sizing/review/Worker-discipline
  rules: Execution Prep must follow the accepted P6 seam classifications;
...
| M02R | `workflow/REVIEW.md`, `workflow/EXECUTION_PREP.md`, `workflow/PLANNING.md`, `workflow/EXECUTION.md`, review/Task Board state contracts and tests | Existing subject-relative review/JIT/execution boundaries stay valid while gaining exhaustive discovery vs closure, defect-class convergence, durable observations, semantic Card right-sizing, seam fidelity/topology challenge, Worker falsification/YAGNI discipline and live-finding reconciliation |
...
Authority input pins for P6 (consumer branch state containing Definition R3):
...
**P6 lifecycle note:** historical terminal milestone. P6 does not reopen or
...
**P6 lifecycle note:** historical terminal milestone. M02 has terminal fresh
...
implemented by this milestone, P6 freezes four **required_seam** boundaries
...
classification against the governed P6 materialization. No special user prompt
...
- Historical GREEN results and correctly closed stages are never
  ...
  (REQ-098/099/125).
- M01/M02 remain terminal. P6 does not reopen them. M02 is additionally
  replayed as immutable regression evidence by M02R.
- The existing waiting `after-M02-T01` JIT state must be reconciled after
  P6 approval: it must not materialize M03 directly. Execution Prep first
  materializes M02R under its four required seams; only after M02R terminal
...
Full P6 audit evidence:
`implementation/workstreams/change-pwv21-policy-kernel-brainstorming/evidence/PLANNING_AUDIT_P6_2026-09-24.md`.

- Definition R3 is GREEN with 137 accepted requirements and 8 accepted ADRs.
- Premium A for `definition:R3|planning-cycle:6` was satisfied before
  material P6 planning.
- M01/M02 remain terminal and are not rematerialized.
- P6 preserves P5 milestone topology; because M02R was not materialized, R3
  is incorporated into that bootstrap rather than creating another milestone.
- M02R has four Planning-level `required_seam` boundaries: BOOT-A review/
  convergence/observation closure, BOOT-B decomposition/right-sizing, BOOT-C
  live validation, and BOOT-D Worker discipline.
- Right-sizing explicitly rejects both whole-milestone mega-Cards and
  meaningless file/layer/test-step micro-Cards.
- P6 closes P5/R01 finding F1 by binding REQ-108 causal-blast-radius closure
  semantics into BOOT-A planned work, acceptance, verification and Appendix A;
  P5 observations O1/O2 are also resolved without changing strategy.
- Review convergence is bound at Planning level: 5/4/3 genuinely-new material
  defect-class discovery epochs, 3 failed repair→closure rounds per material
  defect class, fresh rediscovery after closure, and structural routing after
  RED post-convergence validation.
- Non-load-bearing review observations are durable and must be dispositioned;
  unresolved observations cannot silently vanish before Final Integration.
- Worker execution is falsification-first where meaningful and otherwise uses
  explicit observable acceptance, with YAGNI binding and DRY non-absolute.
- Original M03–M07 semantic outcomes remain in order; M03 is the first
  downstream authority-level dogfood of complete R2+R3 semantics.
- Requirement inventory is 137/137 unique, 0 missing, 0 duplicate.
- All eight accepted ADRs have explicit owning milestone coverage.
- The waiting `after-M02-T01` trigger cannot materialize M03 directly; after
  P6 approval it must be reconciled to M02R-first execution with four required
  bootstrap seams.
- P6 preserves authority-level dogfood vs deployed workflow enforcement while
  `project_workflow_v2@main` remains canonical.
- Planner completeness/challenge audit: GREEN. Independent Stage-6 Plan Review
  remains pending and must occur only after Premium B in a fresh independent
  context.
