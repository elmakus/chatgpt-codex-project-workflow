...
Pointer ownership and lifecycle are defined by the selected policy route, **not by this shared template**. Under `chatgpt_only`, pre-execution Research uses the selected workstream manifest `routing.research_obligation` and implementation/recovery Research uses the selected canonical Task Board `research_obligation`; ordinary classifier routing keeps the current record `complete` until a final owning Return target durably reconciles the findings, while the policy-local classifier-to-Research chain may instead atomically consume that record and replace the pointer with the exact next active Research obligation. A legacy/other-policy route that does not define these pointer semantics must not create manifest/Task-Board Research routing state merely because this template exposes optional continuation metadata. Do not infer Origin/Return from chat history.
