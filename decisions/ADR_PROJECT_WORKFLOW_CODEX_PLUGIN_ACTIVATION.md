...
### Three Skills (`project-workflow`, `pw-issue`, `pw-feature`)
...
- Evidence/research: M01-T01 runtime evidence proved bundled Skills are exposed as `<plugin-name>:<skill-name>` and that literal `$pw` does not invoke a bundled Skill.
- Naming-constraint evidence: `openai/codex@5c5308fc9a9ee789049d646ef11e5400384b9c6f`, `codex-rs/skills/src/assets/samples/skill-creator/scripts/quick_validate.py`, normal Skill-name regex `^[a-z0-9-]+$`.
- User/product reconciliation: on 2026-09-20 the user first accepted `$pw:pw`, later reopened the choice for readability/autocomplete and requested `$pw:project_workflow`; after the hyphen-case constraint was surfaced, the user explicitly accepted `$pw:project-workflow`.
- Final accepted explicit identity: plugin `pw` + Skill `project-workflow` → `$pw:project-workflow`.
