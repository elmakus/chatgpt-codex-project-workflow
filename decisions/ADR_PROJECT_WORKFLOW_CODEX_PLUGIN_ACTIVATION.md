...
2. one explicit concise Project Workflow bundled Skill using the verified current Codex identity `$pw:project-workflow` (plugin `pw` + Skill `project-workflow`), which routes into canonical Project Workflow and can accept subcommands/arguments when needed.
...
Prefer existing `#issue` and `#feature` directives when end-to-end tests prove they reliably reach the canonical intake route under this always-on model. If that reliability is not demonstrated, use `$pw:project-workflow issue ...` and `$pw:project-workflow feature ...` as the documented fallback. Do not create separate duplicate Skills merely to provide those aliases.
...
- Usage docs must record the verified intake syntax after comparing `#issue/#feature` with the `$pw:project-workflow` fallback.
...
- Evidence/research: current Codex plugin/Skill/hook capabilities and existing Project Workflow routing; M01-T01 runtime evidence proved bundled Skills are exposed as `<plugin-name>:<skill-name>` and that literal `$pw` does not invoke a bundled Skill. The accepted explicit identity is now plugin `pw` + Skill `project-workflow` → `$pw:project-workflow`, with the Skill name constrained by current hyphen-case validation.
- User/product reconciliation: on 2026-09-20 the user first accepted `$pw:pw`, then explicitly reopened the command choice for readability/autocomplete and requested `$pw:project_workflow`. Current Codex Skill validation was verified to allow lowercase letters, digits and hyphens only; the user then explicitly accepted the conforming command `$pw:project-workflow`.
- Naming-constraint evidence: `openai/codex@5c5308fc9a9ee789049d646ef11e5400384b9c6f`, `codex-rs/skills/src/assets/samples/skill-creator/scripts/quick_validate.py`, regex `^[a-z0-9-]+# Decision — Project Workflow Codex plugin uses minimal always-on activation plus one explicit Skill
...
- Source discussion/request: user-authorized `project-workflow-codex-plugin@R1`.
- Evidence/research: current Codex plugin/Skill/hook capabilities and existing Project Workflow routing; M01-T01 runtime evidence proved bundled Skills are exposed as `<plugin-name>:<skill-name>` and that plugin `pw` + Skill `pw` yields `$pw:project-workflow`, while literal `$pw` does not invoke the bundled Skill.
.
