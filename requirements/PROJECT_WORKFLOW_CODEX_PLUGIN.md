...
| PWCP-REQ-003 | Provide exactly one normal explicit Project Workflow Skill entrypoint, named for concise invocation as `$pw` unless current Codex naming constraints require an equivalent equally short supported name. | MUST | brainstorming R1 / ADR-PWCP-002 | accepted |
...
| PWCP-REQ-009 | If `#issue` / `#feature` are not reliably equivalent to explicit Skill routing, the supported fallback UX must be one Skill with arguments: `$pw issue ...` and `$pw feature ...`; do not introduce separate duplicate `pw-issue` and `pw-feature` Skills merely for aliases. | MUST | user choice | accepted |
| PWCP-REQ-010 | Keep `$pw` as a general explicit entry/recovery/debug path regardless of which intake-directive UX wins verification. | MUST | user choice | accepted |
...
6. Short operator UX is preserved when reliability permits it.
...
1. The plugin can be installed/enabled from its Git-backed marketplace source and its one `$pw` Skill is discoverable.
...
5. `$pw` explicitly enters/re-enters the workflow.
6. `#feature` and `#issue` are tested end-to-end against `$pw feature` and `$pw issue`; durable usage docs record the verified convention.
