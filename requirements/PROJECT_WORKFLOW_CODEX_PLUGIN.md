...
| PWCP-REQ-003 | Provide exactly one normal explicit Project Workflow bundled-Skill entrypoint. Under the verified current Codex namespacing contract, use plugin name `pw` + Skill name `project-workflow`, exposed as `$pw:project-workflow`. | MUST | brainstorming R1 / ADR-PWCP-002 / runtime evidence 2026-09-20 / user decision 2026-09-20; hyphen-case naming constraint verified 2026-09-20 | accepted |
| PWCP-REQ-004 | Enabling the plugin for a repository must establish a lightweight always-on Project Workflow invariant for ordinary user prompts; the user must not have to invoke `$pw:project-workflow` on every message. | MUST | brainstorming R1 / ADR-PWCP-002 | accepted |
...
| PWCP-REQ-009 | If `#issue` / `#feature` are not reliably equivalent to explicit Skill routing, the supported fallback UX must be the one bundled Skill with arguments: `$pw:project-workflow issue ...` and `$pw:project-workflow feature ...`; do not introduce separate duplicate issue/feature Skills merely for aliases. | MUST | user choice + current Codex naming constraint + user decision accepted 2026-09-20 | accepted |
| PWCP-REQ-010 | Keep `$pw:project-workflow` as the general explicit entry/recovery/debug path on the verified current Codex bundled-Skill naming contract, regardless of which intake-directive UX wins verification. | MUST | user choice + current Codex naming constraint accepted 2026-09-20 | accepted |
...
- Normal Skill names must satisfy the verified current Codex hyphen-case validation contract; the accepted Skill name is `project-workflow`, not `project_workflow`.
...
- Creating three independent Skills only to provide `project-workflow`, issue and feature aliases.
...
6. Short operator UX is preserved when reliability permits it; the verified/accepted current bundled-Skill command is `$pw:project-workflow`.
...
1. The plugin can be installed/enabled from its Git-backed marketplace source and its one `$pw:project-workflow` bundled Skill is discoverable.
2. A normal prompt in an enabled test repository enters Project Workflow without explicit `$pw:project-workflow` invocation.
...
5. `$pw:project-workflow` explicitly enters/re-enters the workflow.
6. `#feature` and `#issue` are tested end-to-end against `$pw:project-workflow feature` and `$pw:project-workflow issue`; durable usage docs record the verified convention.
...
- The reopened explicit-command choice is resolved: user/product authority accepted `$pw:project-workflow`, which conforms to the verified normal Skill-name constraint.
