...
- `chatgpt_only`: ChatGPT is fixed Task Card executor. Do not run Capability Gate or capability preflight/inventory. Run state/contract Refresh Gate, then attempt the work. If a concrete required operation cannot be performed, persist a runtime blocker and ask for the smallest remedy. If REQUIRED/RECOMMENDED independent review is due for a subject implemented by the current chat, persist exact review subject/state and STOP so the user can open a fresh normal ChatGPT chat for review. Whenever you require or recommend a fresh ChatGPT chat, include in the same response a minimal fenced copy-paste start prompt containing the project repo, exact active branch, exact continuation target and durable pointer; do not duplicate durable state already recoverable from the repo.
- `codex_only`: Codex is fixed Task Card executor. Do not run Capability Gate or capability inventory/preflight. Project Workflow does not prescribe Codex's tool inventory; Codex starts directly with its actual runtime, handles ordinary executor-local remediation when permitted, and asks for the smallest user-provided input/access/authorization only when a concrete required operation still cannot proceed. Codex may continue across already-approved GREEN milestone boundaries when no strategic/user/authorization gate intervenes. When `codex_workflow` is installed/enabled, it governs internal execute/review-worker orchestration.
